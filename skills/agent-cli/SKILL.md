...
For API-backed CLIs, accept the full API payload via `--json` or `--params` alongside any convenience flags. This eliminates translation loss between what the agent generates and what the API accepts.

**Design pattern:**
```
mycli resource create --json '{"field": "value", "nested": {"key": "val"}}'
mycli resource list --params '{"filter": "status=active", "pageSize": 50}'
```

**Priority rules when both JSON and flags are provided:**
...
Convenience flags (`--title`, `--name`) remain for human use but are secondary to the raw payload path.
