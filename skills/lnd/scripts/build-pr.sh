...
# 2. Builds with a tag like lnd:pr-<number>
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo "Examples:"
    echo "  build-pr.sh 1234    # Build PR #1234"
    exit 1
fi
...
echo "Building lnd from PR #$PR_NUMBER..."
echo ""

"$SCRIPT_DIR/docker-build.sh" --pr "$PR_NUMBER" --tag "lnd:pr-$PR_NUMBER" "$@"

echo ""
echo "To run with docker-compose using this image:"
echo "  LND_IMAGE=lnd:pr-$PR_NUMBER docker-compose up -d"
